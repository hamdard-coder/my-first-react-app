...
var MY_LEARNING_VERSION = '1.0.106'; // dynamically set in distribution files
...
/*
// TEMP: nextprev presence probe — remove after debugging
(function () {
  ...
  function logProbe(label, payload) {
    if (typeof Util !== 'undefined' && typeof Util.logDebug === 'function') {
      Util.logDebug('MyLearning', label, payload);
    } else {
  ...
  function tick() {
    ...
    var count = document.querySelectorAll('.nextprev').length;
    var payload = {
      elapsedMs: elapsedMs,
      ticks: ticks,
      count: count,
    ...
    logProbe('TEMP nextprev probe', payload);

    if (count > 0) {
      logProbe('TEMP nextprev probe -> FOUND', payload);
      stopped = true;
      return;
    }
    ...
    // stop after ~5s
    if (elapsedMs >= 5000) {
      logProbe('TEMP nextprev probe -> gave up', payload);
      stopped = true;
      return;
    }

    // setTimeout(0) = max timer resolution (sync sample already ran at t=0)
    timeoutId = setTimeout(tick, 0);
  }

  // immediate sample before any timer delay
  tick();
})();
*/
...
window.MyLearning = {
  ...
  _spacesLabsApiBaseUrl: 'https://spaces-labs-api.w3sapi.com',
  ...
  _sidenavProgressListenerRegistered: false,
  loadSharedApiModuleForAuthenticatedUser: null,
  _userStateFetchInFlight: null,
  _userStateCache: null,
  // Extra gates that postpone pushProgress until isReady() (see registerProgressPushGate).
  _progressPushGates: [],
  _progressTrackingDependentModulesAttached: false
};
...
MyLearning.extraFeaturesEnabled = function (params) {
  try {
    ...
    if (!UserSession.userHasExtraFeaturesAccess()) {
      MyLearning._logDebug('RhsMenu -> ExtraFeaturesEnabled -> false: user is not paying (not logged in or on free/closed plan without full access)');
      return false;
    }
    if (!params.skipMainElementCheck) {
      var main = document.getElementById('main');
      if (!main) {
        MyLearning._logDebug('RhsMenu -> ExtraFeaturesEnabled -> false: main element not found');
        return false;
      }
    }
    ...
    '/academy/', '/adventure/', '/bootcamp/', '/training/', '/spaces/', '/league/', '/videos/', '/typingspeed/', '/forum/', '/leaderboard/', '/newsletter/', '/pathfinder/', '/plus/', '/signup/'];

    // Check for exact matches
    if (skipPaths.indexOf(path) !== -1) {
      MyLearning._logDebug('RhsMenu -> ExtraFeaturesEnabled -> false: path is in skipPaths', {
        path: path
      });
      return false;
    }
    ...
    for (var i = 0; i < skipPaths.length; i++) {
      if (skipPaths[i] !== '/' && skipPaths[i].endsWith('/') && path.startsWith(skipPaths[i])) {
        MyLearning._logDebug('RhsMenu -> ExtraFeaturesEnabled -> false: path starts with skipPath', {
          path: path,
    ...
    if (pathParts.length === 0) {
      MyLearning._logDebug('RhsMenu -> ExtraFeaturesEnabled -> false: no meaningful path parts', {
        path: path
      });
      return false;
    }
    ...
    if (MyLearning.pageType && ['page', 'lesson'].indexOf(MyLearning.pageType) !== -1 && !topicMap[pathTopic]) {
      MyLearning._logDebug('RhsMenu -> ExtraFeaturesEnabled -> false: not a known topic for page/lesson type', {
    ...
    MyLearning._logDebug('RhsMenu -> ExtraFeaturesEnabled -> true: all checks passed', {
      path: path,
      pathTopic: pathTopic,
      pageType: MyLearning.pageType
    });
...
MyLearning._isClassicWebsite = function () {
  try {
    var w3sHostname = new URL(MyLearning._w3sBaseUrl).hostname;
    var hostname = window.location.hostname;
    var isClassicHost = hostname === w3sHostname || hostname === 'classic-local.w3schools.com' || hostname === 'localhost.w3schools.com';
    return isClassicHost;
  } catch (e) {
    return false;
  }
};
  ...
  try {
    ...
    if (MyLearning.extraFeaturesEnabled({
      ...
      try {
        ...
        console.error('MyLearning -> RhsMenu -> ImmediateStyle -> error (1): ', e);
      }

      // Inject CSS IMMEDIATELY in head to prevent flash
      if (!document.getElementById('rhs-menu-immediate-style')) {
        MyLearning._logDebug('RhsMenu -> ImmediateStyle -> injecting rhs-menu-immediate-style');
    ...
    console.error('MyLearning -> RhsMenu -> ImmediateStyle -> error (2): ', e);
  }
})();
...
MyLearning._iframeInit = function () {
  ...
  window.addEventListener('message', function (message) {
    ...
    var whitelistedOrigins = ['https://profile.w3schools.com', 'https://pathfinder.w3schools.com', 'https://my-learning.w3schools.com', 'https://spaces.w3schools.com', 'https://campus.w3schools.com', 'https://order.w3schools.com'
...
// Temporary: declined-trial is async via billing API response.
// Replace with a synchronous check from UIC once that cookie carries the state.
MyLearning.isProgressTrackingEnabled = function () {
  return UserSession.hasActiveDeclinedTrial({
    context: 'MyLearning -> isProgressTrackingEnabled'
  }) !== true && UserSession.getUserSubscriptionPlan() !== 'closed';
};

// Temporary: declined-trial needs async billing; re-sync UI when that data arrives.
// Replace with a synchronous check from UIC once that cookie carries the state.
MyLearning._isProgressTrackingEligibilityKnown = function () {
  if (typeof UserSession === 'undefined' || UserSession.loggedIn !== true) {
    return true;
  }
  if (UserSession.getUserSubscriptionPlan() === 'closed') {
    return true;
  }
  return UserSession._cachedBillingAccountBasic !== null;
};

// Temporary: hide profile-circle progress, refresh dropdown/bottom lock, and re-patch next/prev after billing loads.
// Replace with a synchronous UIC-based solution so no post-load sync is needed.
MyLearning._syncProgressTrackingUi = function () {
  if (typeof NextButtonModal !== 'undefined' && typeof NextButtonModal._refreshNextButtonInterceptWhenBillingReady === 'function') {
    NextButtonModal._refreshNextButtonInterceptWhenBillingReady();
  }
  if (!MyLearning._isProgressTrackingEligibilityKnown()) {
    return;
  }
  var showProgress = MyLearning.isProgressTrackingEnabled();
  MyLearning._findInnerElements(document, '.user-progress', function (elm) {
    elm.style.display = showProgress ? '' : 'none';
  });
  MyLearning._findInnerElements(document, '.user-progress-star', function (elm) {
    if (!showProgress) {
      elm.style.display = 'none';
      elm.classList.remove('up-active');
    } else {
      elm.style.display = '';
    }
  });
  MyLearning._findInnerElements(document, '.user-progress-point', function (elm) {
    if (!showProgress) {
      elm.style.display = 'none';
      elm.classList.remove('up-active');
    } else {
  ...
  if (typeof TopNavBar !== 'undefined' && typeof TopNavBar._syncProfileDropdownProgressVisibility === 'function') {
    TopNavBar._syncProfileDropdownProgressVisibility();
  }
  if (typeof BottomStatus !== 'undefined') {
    if (typeof BottomStatus._ensurePlacement === 'function') {
      BottomStatus._ensurePlacement();
    }
    if (typeof BottomStatus._tryPlaceOrRender === 'function' && !BottomStatus._rendered) {
      BottomStatus._tryPlaceOrRender();
    }
    if (typeof BottomStatus._checkAndApplyLockedState === 'function') {
      BottomStatus._checkAndApplyLockedState();
    }
  }
};

// Temporary: prefetch billing so declined-trial UI can re-sync after API response.
// Replace with a synchronous UIC-based solution so no prefetch callback is needed.
MyLearning._prefetchBillingAccountBasic = function (context) {
  if (typeof UserSession === 'undefined' || typeof UserSession.cacheBillingAccountBasic !== 'function') {
    return;
  }
  if (UserSession.getUicCookie() === null) {
    return;
  }
  UserSession.cacheBillingAccountBasic({
    context: context,
    callback: function callback(res) {
      MyLearning._logDebug('_prefetchBillingAccountBasic -> context, res: ', {
        context: context,
        res: res
      });
      MyLearning._syncProgressTrackingUi();
    }
  });
};
...
MyLearning._apiNameToPathMap = {
  ...
  'api.project.submit': '/api/classic/set-project-progress',
  'api.project.progress': '/api/classic/get-project-progress'
};
    ...
    if (reqRes.error.code === '0' && reqRes.status === 200) {
      ...
      if (x == 'F' || x == 'G' || x == 'H' || x == 'I' || x == 'J' || x == 'K' || x == 'L' || x == 'M' || x == 'Q') {
        ...
        if (reqResDataParseRes.error.code === '0') {
          ...
          MyLearning._uiSidenavApplyCompletedProgress(reqResData.completed);
...
// Gate shape: { isReady(): boolean, whenReady(onReady: function): void }
MyLearning.registerProgressPushGate = function (gate) {
  if (gate && typeof gate.isReady === 'function' && typeof gate.whenReady === 'function') {
    MyLearning._progressPushGates.push(gate);
  }
};
MyLearning._getProgressPushGates = function () {
  var gates = MyLearning._progressPushGates.slice();
  try {
    var pe = window.ProjectEngine;
    if (pe && typeof pe.isProjectCompleted === 'function' && typeof pe.onProjectCompleted === 'function') {
      gates.push({
        isReady: function isReady() {
          var status = pe.isProjectCompleted();
          return !!(status && status.result === true);
        },
        whenReady: function whenReady(onReady) {
          pe.onProjectCompleted('MyLearning:' + (window.location.pathname || ''), function (callbackRes) {
            try {
...
MyLearning._whenAllProgressPushGatesReady = function (gates, onReady) {
  var waiting = 0;
  var i;
  ...
  for (i = 0; i < gates.length; i++) {
    if (!gates[i].isReady()) {
      gates[i].whenReady(gateReady);
    }
  }
};
MyLearning._applyLocalPageCompletionForCurrentPath = function () {
  var currentPath = Util.removeStrPrefix(window.location.pathname, '/');
  if (MyLearning.pageType === 'page') {
...
MyLearning._onProgressPushRecordedForCurrentPage = function (responseCode) {
  if (responseCode !== 'O') {
    return;
  }
  if (MyLearning.pageType === 'lesson') {
    MyLearning.registerPointForFinishedLesson(responseCode);
  }
  MyLearning._applyLocalPageCompletionForCurrentPath();
};
  ...
  function pushProgressForCurrentPage(prs) {
    ...
    MyLearning.pushProgress(MyLearning.getUrl('api.meta'), reqDataStr, function (reqRes) {
      if (reqRes.error.code === '0' && reqRes.status === 200) {
        ...
        var x = y.substr(0, 1);
        if (x == 'O') {
          MyLearning._onProgressPushRecordedForCurrentPage(x);
        }
      }
    });
  }
  var gates = MyLearning._getProgressPushGates();
  MyLearning._whenAllProgressPushGatesReady(gates, function () {
    pushProgressForCurrentPage({
      context: gates.length ? 'GATES_READY' : 'IMMEDIATE'
    });
  });
};
...
MyLearning.uiSidenavPatch = function () {
  ...
  if (UserSession.loggedIn === true) {
    if (typeof MyLearning.lessonMeta !== 'undefined' && Array.isArray(MyLearning.lessonMeta.completed)) {
      MyLearning._uiSidenavApplyCompletedProgress(MyLearning.lessonMeta.completed);
    } else {
      MyLearning._registerSidenavProgressListener();
      MyLearning._uiSidenavEnableCheckmarks();
    }
  }
};
MyLearning._uiSidenavApplyCompletedProgress = function (completed) {
  if (UserSession.loggedIn !== true) {
    return;
  }
  MyLearning._uiSidenavEnableCheckmarks();
  if (Array.isArray(completed) && completed.length) {
    MyLearning._uiSidenavFlagCompletedLinks(completed);
  }
};
MyLearning._registerSidenavProgressListener = function () {
  if (MyLearning._sidenavProgressListenerRegistered) {
    return;
  }
  MyLearning._sidenavProgressListenerRegistered = true;
  Util.objFieldOnSetCallback({
    scopeRef: MyLearning,
    fieldName: 'lessonStateFetched',
    fieldValue: true,
    fieldValueEquals: true,
    callback: function callback(res) {
...
MyLearning._loadRhsMenuIfEnabled = function () {
  if (MyLearning._rhsMenuLoadAttempted) {
    return;
  }
  if (!document.getElementById('main') && document.readyState === 'loading') {
    MyLearning._logDebug('RhsMenu -> waiting for #main before loading script and style');
    document.addEventListener('DOMContentLoaded', function () {
      MyLearning._loadRhsMenuIfEnabled();
    }, {
      once: true
    });
    return;
  }
  MyLearning._rhsMenuLoadAttempted = true;
  var rhsExtraFeaturesEnabled = MyLearning.extraFeaturesEnabled();
  var rhsIsClassicWebsite = MyLearning._isClassicWebsite();
  if (rhsExtraFeaturesEnabled && rhsIsClassicWebsite) {
    MyLearning._logDebug('RhsMenu -> loading script and style');
    ...
    Util.attachCss({
      ...
      callback: function callback(res) {
        MyLearning._logDebug('RhsMenu -> Style -> loaded -> res: ', res);
      }
    });
    ...
    Util.attachScript({
      ...
      callback: function callback(res) {
        MyLearning._logDebug('RhsMenu -> Script -> loaded -> res: ', res);
        if (res.error.code === '0') {
          if (typeof RightHandSideMenu !== 'undefined' && RightHandSideMenu.init) {
            RightHandSideMenu.init();
          }
        } else {
          MyLearning._logError('RhsMenu -> Script -> failed loading -> error: ', res.error);
        }
      }
    });
  } else {
    MyLearning._logDebug('RhsMenu -> skipped loading script and style', {
      extraFeaturesEnabled: rhsExtraFeaturesEnabled,
      isClassicWebsite: rhsIsClassicWebsite
    });
  }
};
...
// Loads BottomStatus, NextButtonModal, and Pathfinder once .nextprev exists.
// Returns true when attach ran (or already ran); false when .nextprev is still missing.
MyLearning._attachProgressTrackingDependentModules = function () {
  if (MyLearning._progressTrackingDependentModulesAttached) {
    return true;
  }
  var nextPrevBtnsWrapperElm = document.querySelectorAll('.nextprev');
  if (nextPrevBtnsWrapperElm.length === 0) {
    MyLearning._logDebug('_attachProgressTrackingDependentModules -> skipped, .nextprev not found', {
      readyState: document.readyState
    });
    return false;
  }
  MyLearning._progressTrackingDependentModulesAttached = true;
  MyLearning._logDebug('_attachProgressTrackingDependentModules -> attaching', {
    nextPrevCount: nextPrevBtnsWrapperElm.length,
    readyState: document.readyState
  });
  MyLearning.loadSharedApiModule();
  if (MyLearning._isClassicWebsite()) {
...
// Head scripts can resolve UserSession.loggedIn before body/.nextprev exist — defer if needed.
MyLearning._scheduleProgressTrackingDependentModules = function () {
  if (MyLearning._attachProgressTrackingDependentModules()) {
    return;
  }
  if (document.readyState === 'loading') {
    MyLearning._logDebug('_scheduleProgressTrackingDependentModules -> waiting for DOMContentLoaded');
    document.addEventListener('DOMContentLoaded', function onDomReady() {
      document.removeEventListener('DOMContentLoaded', onDomReady);
      MyLearning._attachProgressTrackingDependentModules();
    });
    return;
  }
  MyLearning._logDebug('_scheduleProgressTrackingDependentModules -> DOM ready, no .nextprev');
};
MyLearning.init = function () {
  ...
  MyLearning._syncProgressTrackingUi();
  MyLearning._prefetchBillingAccountBasic('MyLearning -> init');
  ...
  Util.objFieldOnSetCallback({
    ...
    callback: function callback(res) {
      ...
      if (res.error.code === '0') {
        ...
        if (UserSession.loggedIn === true) {
          MyLearning._prefetchBillingAccountBasic('MyLearning -> loggedIn');
          if (MyLearning.loadSharedApiForAuthenticatedUser === true) {
            MyLearning.loadSharedApiModule();
          }
          MyLearning._registerSidenavProgressListener();

          // RHS stays gated behind paid/extra-features access
          MyLearning._loadRhsMenuIfEnabled();
          MyLearning._scheduleProgressTrackingDependentModules();
        }
      } else {
...
// Full struct path for classic project-engine pages, e.g. html/html_project_first.asp
MyLearning.getClassicStructPath = function () {
...
// options: { projectId, projectUuid, topic, path, pathUuid }
//   projectId   - lowercase struct id, e.g. 'htmlpage001' (preferred for projects workspace)
//   projectUuid - alternative lookup key
//   topic       - optional; topic-level pct / completedCodes (projectUuid keys)
// With projectId or projectUuid: response may include completion { version, currentStep, awaitingContinue, fileSnapshots, stepChangeDeltas, completed, ... }
MyLearning.fetchProjectProgress = function (options, _callback4) {
  if (typeof options === 'function') {
    _callback4 = options;
    options = {};
  }
  ...
  if (options && options.projectUuid) {
    params.push('projectUuid=' + encodeURIComponent(options.projectUuid));
  }
  if (options && options.path) {
    params.push('path=' + encodeURIComponent(options.path));
  }
  if (options && options.pathUuid) {
    params.push('pathUuid=' + encodeURIComponent(options.pathUuid));
  }
  var url = MyLearning.getUrl('api.project.progress') + (params.length ? '?' + params.join('&') : '');
  MyLearning._logDebug('fetchProjectProgress -> url: ', url);
  Util.fetch({
    context: 'MyLearning -> fetchProjectProgress',
    method: 'GET',
    url: url,
    callback: function callback(res) {
      MyLearning._logDebug('fetchProjectProgress -> response: ', res);
      if (typeof _callback4 === 'function') {
        _callback4(res);
      }
    },
    withUserSession: true,
...
// progressData: { projectId, version, currentStep, awaitingContinue, fileSnapshots?, submittedStep?, hintsUsed?, completed?, finishModalDismissed?, welcomeHidden? }
//   projectId             - required lowercase struct id, e.g. 'htmlpage001'
//   version               - progress schema version (required; currently 2)
//   currentStep           - zero-based active step index (required)
//   awaitingContinue      - true after a successful check before Continue (required)
//   fileSnapshots         - optional { [fileId]: { codeSnapshot, changeDelta? } }; changeDelta only on successful step check
//   submittedStep         - optional zero-based step index for deltas (default: currentStep when awaitingContinue)
//   hintsUsed             - optional { "0": true, ... } steps where the hint panel was opened before a successful check
//   completed             - optional; true only on final project finish
//   finishModalDismissed  - optional UI flag
//   welcomeHidden         - optional UI flag
// Apply server-reported XP (xpAssigned) to visible profile widgets.
MyLearning.applyXpAssigned = function (amount) {
  var xp = parseInt(amount, 10);
  if (isNaN(xp) || xp <= 0) {
    return;
  }
  if (!MyLearning.isProgressTrackingEnabled()) {
    MyLearning._logDebug('applyXpAssigned -> skipping: progress tracking disabled');
    return;
  }
  MyLearning._logDebug('applyXpAssigned -> amount: ', xp);
  if (typeof BottomStatus !== 'undefined' && typeof BottomStatus.incrementXp === 'function') {
    BottomStatus.incrementXp(xp);
  }
  if (typeof RightHandSideMenu !== 'undefined' && typeof RightHandSideMenu.incrementXp === 'function') {
    RightHandSideMenu.incrementXp(xp);
  } else if (typeof TopNavBar !== 'undefined' && TopNavBar.mylUserLiteState && typeof TopNavBar.mylUserLiteState.xp === 'number') {
    TopNavBar.mylUserLiteState.xp += xp;
  }
  try {
    if (typeof SharedAPI !== 'undefined' && SharedAPI._config && SharedAPI._config.cache) {
      if (SharedAPI._config.cache.xp && typeof SharedAPI._config.cache.xp.xp === 'number') {
        SharedAPI._config.cache.xp.xp += xp;
      } else {
        SharedAPI._config.cache.xp = null;
        if (SharedAPI._config.cache.lastFetch) {
          delete SharedAPI._config.cache.lastFetch.xp;
        }
      }
    }
  } catch (_cacheErr) {
...
// Called after each slide completion and on final finish (completed: true).
// Response data: { xpAssigned: number, completed: boolean }
MyLearning.submitProjectProgress = function (progressData, _callback5) {
  MyLearning._logDebug('submitProjectProgress -> progressData: ', progressData);
  if (UserSession.loggedIn !== true) {
    MyLearning._logDebug('submitProjectProgress -> skipping: user not logged in');
    return;
  }
  if (!MyLearning.isProgressTrackingEnabled()) {
    MyLearning._logDebug('submitProjectProgress -> skipping: progress tracking disabled');
    return;
  }
  var userInfoCookie = UserSession.getUicCookie();
  if (!userInfoCookie) {
    MyLearning._logError('submitProjectProgress -> userInfoCookie not found');
    return;
  }
  var subscriptionPlan = userInfoCookie.plan ? userInfoCookie.plan.toLowerCase() : 'free';
  if (['closed'].includes(subscriptionPlan)) {
    MyLearning._logWarning('submitProjectProgress -> skipping: user is on closed plan');
    return;
  }
  if (!progressData || !progressData.projectId) {
    MyLearning._logError('submitProjectProgress -> missing projectId');
    return;
  }
  if (typeof progressData.currentStep !== 'number') {
    MyLearning._logError('submitProjectProgress -> missing currentStep');
    return;
  }
  if (typeof progressData.awaitingContinue !== 'boolean') {
    MyLearning._logError('submitProjectProgress -> missing awaitingContinue');
    return;
  }
  if (typeof progressData.version !== 'number') {
    MyLearning._logError('submitProjectProgress -> missing version');
    return;
  }
  var body = {
    ...
    awaitingContinue: progressData.awaitingContinue
  };
  if (progressData.completed === true) {
    body.completed = true;
  }
  if (typeof progressData.submittedStep === 'number') {
    body.submittedStep = progressData.submittedStep;
  }
  if (progressData.finishModalDismissed === true) {
    body.finishModalDismissed = true;
  }
  if (progressData.welcomeHidden === true) {
    body.welcomeHidden = true;
  }
  if (progressData.hintsUsed && Object.keys(progressData.hintsUsed).length > 0) {
    body.hintsUsed = progressData.hintsUsed;
  }
  if (progressData.fileSnapshots && Object.keys(progressData.fileSnapshots).length > 0) {
    body.fileSnapshots = progressData.fileSnapshots;
  }
  MyLearning._logDebug('submitProjectProgress -> body: ', body);
  Util.fetch({
    context: 'MyLearning -> submitProjectProgress',
    method: 'POST',
    url: MyLearning.getUrl('api.project.submit'),
    ...
    callback: function callback(res) {
      MyLearning._logDebug('submitProjectProgress -> response: ', res);
      try {
        if (res && res.error && res.error.code === '0' && res.data && typeof res.data.xpAssigned !== 'undefined') {
          MyLearning.applyXpAssigned(res.data.xpAssigned);
        }
      } catch (_xpErr) {
        // swallow
      }
      if (typeof _callback5 === 'function') {
        _callback5(res);
      }
    }
  });
