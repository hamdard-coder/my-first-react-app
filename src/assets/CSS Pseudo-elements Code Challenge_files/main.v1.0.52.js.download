...
// GET /api/v3/account?view=basic (billing-stripe-api.w3sapi.com)
//
// Callback receives res.data shaped as { customer, currentSubscription? }.
// Subscription plan and trial/onboarding state are read from these fields.
// cacheBillingAccountBasic fetches and caches res.data for hasActiveDeclinedTrial.
// (see fetchBillingAccountBasic, _resolveSubscriptionPlanFromBillingAccountBasic, hasActiveDeclinedTrial).
//
// customer — always present when logged in:
//   firstname, lastname, email, cognito_username, cognito_sub, stripe_id,
//   is_payment_collection_active, partition, created_at, updated_at
//
// customer trial / onboarding flags (vary by user state):
//   has_declined_free_trial, free_trial_declined_at — user chose "Not now" during onboarding
//   has_used_free_trial, free_trial_ended_at — user started a free trial that has ended
//   should_downgrade_to_closed — trial/sub lapsed; may appear before or with a closed subscription
//
// currentSubscription — present when the user has a billing subscription record:
//   status, price_slug, group_slug, product_type_slug, stripe_id, frozen,
//   started_at, next_billing_date, created_at, updated_at, …
//
// Typical combinations:
//   Paid (e.g. plus yearly): currentSubscription.status "active", price_slug "plus-yearly"
//   Declined trial (onboarding): has_declined_free_trial true, no currentSubscription
//   Closed (trial/sub ended): currentSubscription.status "active", price_slug "closed";
//     often with has_used_free_trial, free_trial_ended_at, should_downgrade_to_closed
//   Free (default): no currentSubscription, no decline/trial flags above

UserSession.fetchBillingAccountBasic = function (prs) {
  // interface IFetchBillingAccountBasicParams {
  //   context: string;
  //   callback: (res) => void;
  // }

  UserSession._logDebug('fetchBillingAccountBasic -> init: ', {
    context: prs.context,
  });

  if (typeof prs.callback !== 'function') {
    UserSession._logError('fetchBillingAccountBasic -> callback is not a function -> context: ', {
      context: prs.context,
    });
    return;
  }

  if (typeof Util === 'undefined' || typeof Util.fetch !== 'function') {
    var utilMissingOutput = {
      error: {
        code: 'FBAB_UTIL_MISSING',
        description: 'Util.fetch is not available',
        meta: {
          context: prs.context,
        },
      },
    ...
    UserSession._logError('fetchBillingAccountBasic -> Util.fetch missing -> context, output: ', {
      context: prs.context,
      output: utilMissingOutput,
    });

    return prs.callback(utilMissingOutput);
  }

  var fetchContext = 'UserSession -> fetchBillingAccountBasic';
  if (prs.context) {
    fetchContext += ' -> ' + prs.context;
  }

  Util.fetch({
    context: fetchContext,
    method: 'GET',
    url: UserSession._billingApiBaseUrl + '/api/v3/account?view=basic',
    prepareResponseData: true,
    withUserSession: true,
    handleUserSessionRefresh: true,
    handleUserSessionRedirection: false,
    reqTimeout: UserSession._fetchTimeout,
    callback: function (reqRes) {
      ...
      if (reqRes.error.code === '0') {
        UserSession._logDebug('fetchBillingAccountBasic -> success -> context, output: ', {
          context: prs.context,
          output: output,
        });
      } else {
        UserSession._logError('fetchBillingAccountBasic -> failed -> context, output: ', {
          context: prs.context,
          output: output,
        });
      }
...
UserSession.cacheBillingAccountBasic = function (prs) {
  // interface ICacheBillingAccountBasicParams {
  //   context: string;
  //   callback: (res) => void;
  // }
  //
  // Fetches GET /api/v3/account?view=basic and stores res.data in _cachedBillingAccountBasic on success.
  // Concurrent calls share one in-flight fetch; later callers receive the same result via callback.

  UserSession._logDebug('cacheBillingAccountBasic -> init: ', {
    context: prs.context,
    cacheBillingAccountBasicState: UserSession._cacheBillingAccountBasicState,
  });

  if (typeof prs.callback !== 'function') {
    UserSession._logError('cacheBillingAccountBasic -> callback is not a function -> context: ', {
      context: prs.context,
    });
    return;
  }
  ...
  var deliver = function (res) {
    ...
    if (typeof Util !== 'undefined' && typeof Util.getDictSnapshot === 'function') {
      output = Util.getDictSnapshot(res);
    }

    prs.callback(output);
  };

  if (UserSession._cacheBillingAccountBasicState.inProgress) {
    UserSession._logDebug('cacheBillingAccountBasic -> awaiting in-flight fetch -> context: ', {
      context: prs.context,
    });

    UserSession._cacheBillingAccountBasicState.awaitingCallbacks.push(deliver);
    return;
  }

  UserSession._cacheBillingAccountBasicState.inProgress = true;

  var cacheContext = 'cacheBillingAccountBasic';
  if (prs.context) {
    cacheContext = prs.context + ' -> ' + cacheContext;
  }

  UserSession.fetchBillingAccountBasic({
    context: cacheContext,
    callback: function (res) {
      if (res.error.code === '0' && res.data) {
        UserSession._cachedBillingAccountBasic = res.data;

        UserSession._logDebug('cacheBillingAccountBasic -> cached -> context: ', {
          context: prs.context,
        });
      }

      UserSession._cacheBillingAccountBasicState.inProgress = false;

      deliver(res);

      var awaitingCallbacks = UserSession._cacheBillingAccountBasicState.awaitingCallbacks;
      UserSession._cacheBillingAccountBasicState.awaitingCallbacks = [];

      for (var i = 0; i < awaitingCallbacks.length; i++) {
        awaitingCallbacks[i](res);
      }
    },
  });
};

// Resolves subscription plan from billingAccountBasic (res.data from fetchBillingAccountBasic).
// Uses currentSubscription only:
// - status === 'active' → plan from price_slug (e.g. plus-yearly → plus, closed → closed)
// - no active sub + customer.should_downgrade_to_closed → closed (pre-downgrade transitional state)
// - otherwise → fallback (default free)
UserSession._resolveSubscriptionPlanFromBillingAccountBasic = function (billingAccountBasic, fallbackResult) {
  if (typeof fallbackResult === 'undefined') {
    fallbackResult = 'free';
  }

  if (!billingAccountBasic) {
    return fallbackResult;
  }

  var currentSubscription = billingAccountBasic.currentSubscription;

  if (
    currentSubscription
    && currentSubscription.status === 'active'
    // && currentSubscription.frozen !== true
  ) {
    var priceSlug = currentSubscription.price_slug;

    if (priceSlug) {
      var dashIdx = String(priceSlug).indexOf('-');

      if (dashIdx > 0) {
        return String(priceSlug).substring(0, dashIdx).toLowerCase();
      }

      return String(priceSlug).toLowerCase();
    }
  }

  if (
    billingAccountBasic.customer
    && billingAccountBasic.customer.should_downgrade_to_closed === true
  ) {
    return 'closed';
  }
...
UserSession._resolveHasActiveDeclinedTrialFromBillingAccountBasic = function (billingAccountBasic) {
  if (!billingAccountBasic || !billingAccountBasic.customer) {
    return false;
  }

  if (billingAccountBasic.customer.has_declined_free_trial !== true) {
    return false;
  }

  // Declined first, then upgraded — or downgraded to closed — not active declined anymore.
  if (UserSession._resolveSubscriptionPlanFromBillingAccountBasic(billingAccountBasic) !== 'free') {
    return false;
  }

  // No subscription and declined free trial (onboarding "Not now").
  return true;
};

UserSession.hasActiveDeclinedTrial = function (prs) {
  // interface IHasActiveDeclinedTrialParams {
  //   context: string;
  //   billingAccountBasic?: object; // res.data from fetchBillingAccountBasic; falls back to cacheBillingAccountBasic
  // }
  //
  // Sync. Active declined trial = customer.has_declined_free_trial AND subscription plan === free.
  //
  // Returns null when billingAccountBasic is missing and no cached billing account is available.
  //
  // Returns false when:
  // - has_declined_free_trial is absent/false
  // - subscription plan is not free (active paid sub, or closed after downgrade)
  //
  // Returns true only when: declined + subscription plan is free

  var billingAccountBasic = null;

  if (typeof prs.billingAccountBasic !== 'undefined' && prs.billingAccountBasic !== null) {
    billingAccountBasic = prs.billingAccountBasic;
  } else if (UserSession._cachedBillingAccountBasic !== null) {
    billingAccountBasic = UserSession._cachedBillingAccountBasic;
  }

  if (!billingAccountBasic) {
    // Temporary: billing may still be in-flight (see cacheBillingAccountBasic). Dev-only
    // debug — not an error. Replace with synchronous UIC once declined-trial is on the cookie.
    UserSession._logDebug('hasActiveDeclinedTrial -> billingAccountBasic missing (awaiting cache) -> context: ', {
      context: prs.context,
    });
  ...
  var output = UserSession._resolveHasActiveDeclinedTrialFromBillingAccountBasic(billingAccountBasic);

  UserSession._logDebug('hasActiveDeclinedTrial -> context, output: ', {
    context: prs.context,
    output: output,
  });
